...
class BaseFilTest
{
	...
	public:
	BaseFilTest(int num_rows, int num_cols, float nan_prob, int depth, int num_trees, float leaf_prob, output_t output, float threshold, float global_bias, algo_t algo, int seed, float tolerance, strategy_t strategy, std::string input_model_file, std::string input_data_file)
	{
  ...
  void SetUp(){
    ...
    predict_on_cpu();
	printf("On GPU\n");
    predict_on_gpu_dense();
    predict_on_gpu_dense_adaptive();
...
ps.num_rows = 100;///////////////////////////////////////////////////////////////MUST BE DELETED!!!!!
...
void generate_data() {
  ...
  *pf = f;
  ...
  void predict_on_gpu_dense_adaptive() {
